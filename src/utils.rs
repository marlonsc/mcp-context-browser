...
impl CollectionUtils {
    ...
    pub fn get_or_else<T, F>(map: &HashMap<String, T>, key: &str, default_fn: F) -> T
    where
        F: FnOnce() -> T,
    {
...
impl ErrorUtils {
    ...
    pub fn not_found_error(resource: &str, identifier: &str) -> String {
        Self::format_error(
            ...
            &format!("No {} found with identifier '{}'", resource.to_lowercase(), identifier)
        )
    }
}
...
impl ConfigurableValidator {
    ...
    pub fn validate_string(&self, s: &str) -> ValidationResult<String> {
        // Check length
        let _ = StringValidator::validate_length(s, self.config.min_length, self.config.max_length)?;

        // Check characters if special chars not allowed
        if !self.config.allow_special_chars {
            let _ = StringValidator::validate_alphanumeric(s)?;
        }
...
mod tests {
    ...
    fn test_string_validator_not_empty() {
        assert!(matches!(StringValidator::validate_not_empty("hello"), ValidationResult::Valid(_)));
        assert!(matches!(StringValidator::validate_not_empty(""), ValidationResult::Invalid(_)));
        assert!(matches!(StringValidator::validate_not_empty("   "), ValidationResult::Invalid(_)));
    }

    #[test]
    fn test_string_validator_length() {
        assert!(matches!(StringValidator::validate_length("hi", 1, 10), ValidationResult::Valid(_)));
        assert!(matches!(StringValidator::validate_length("", 1, 10), ValidationResult::Invalid(_)));
        assert!(matches!(StringValidator::validate_length("this_is_a_very_long_string", 1, 10), ValidationResult::Invalid(_)));
    }
    ...
    fn test_configurable_validator() {
        ...
        assert!(matches!(validator.validate_string("hello"), ValidationResult::Valid(_)));
        assert!(matches!(validator.validate_string("hi"), ValidationResult::Invalid(_))); // too short
        assert!(matches!(validator.validate_string("hello@world"), ValidationResult::Invalid(_))); // special char
    }
