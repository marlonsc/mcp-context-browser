...
impl IndexCodebaseHandler {
    ...
    pub async fn handle(
        ...
        if let Err(e) = self.auth_handler.check_auth(token.as_ref(), &Permission::IndexCodebase) {
        ...
        match result {
            Ok(Ok(chunk_count)) => {
                Ok(ResponseFormatter::format_indexing_success(
                    chunk_count,
                    path,
                    duration,
                ))
            ...
            Err(_) => {
                Ok(ResponseFormatter::format_indexing_timeout(path))
            }
        }
    }
}