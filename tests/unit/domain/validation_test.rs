...
//!
//! Migrated from src/domain/validation.rs inline tests.
//! Tests composable validators for strings and numbers.

use mcp_context_browser::domain::validation::{
    common, NumberValidator, NumberValidatorTrait, StringValidator, StringValidatorTrait,
    ValidationError,
};
...
fn test_validation_error_too_long_display() {
    ...
    assert_eq!(
        error.to_string(),
        "Field 'description' is too long: 150 > 100"
    );
}
...
fn test_validation_error_invalid_format_display() {
    ...
    assert_eq!(
        error.to_string(),
        "Field 'email' has invalid format. Expected: valid email format"
    );
}
...
fn test_validation_error_out_of_range_display() {
    ...
    assert_eq!(
        error.to_string(),
        "Field 'age' value '200' is out of range [0, 150]"
    );
}

#[test]
fn test_string_validator_combine_with() {
    let validator = StringValidator::not_empty()
    ...
    assert!(validator.validate("this is too long").is_err()); // too long
}
